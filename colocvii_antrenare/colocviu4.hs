...
expr (If e1 e2) (m1,m2) = if m1 == 0 || m2 == 0 then expr e1 (m1,m2) else expr e2 (m1,m2)
expr (V x) env = x 
...
stmt' :: Stmt -> Env' -> Int
stmt' Off (m1,m2,m3) = m3
stmt' (M Mem1 :<< st) (m1,m2,m3) = stmt' st (expr (M Mem1) (m1,m2),m2,m3+2)
stmt' (M Mem1 :< st) (m1,m2,m3) = stmt' st (m1,expr (M Mem1) (m1,m2),m3+2)
stmt' (M Mem2 :<< st) (m1,m2,m3) = stmt' st (expr (M Mem2) (m1,m2),m2,m3+2)
stmt' (M Mem2 :< st) (m1,m2,m3) = stmt' st (m1,expr (M Mem2) (m1,m2),m3+2)
