...
stmt (Pop) (top:env) = env 
stmt (Pop) [] = error "prea putine erori"


stmt (Plus) (x:y:env) = [x+y]++env
stmt (Plus) env = error "prea putine valori"

stmt (Dup) (x:env) = [x]++(x:env)
stmt (Dup) [] = error "prea putine valori"

stmt _ env = env

stmts :: [Stmt] -> Env -> Env
stmts (Loop [st] : sts) env = if (length env == 1) then stmts sts env else stmts (Loop [st] : sts) (stmt st env)
